...
def replace_kernel(test_session, password=None):
    ...
    if test_session.host:
        ...
        for item in [file_to_send_vmlinuz, file_to_send_modules]:
            test_session.scp_send(item, clr_tmp_dst, password, recursive=True, timeout=20)
        # 拷贝文件
        test_session.scp_send(CWD + clr_rp_exe, clr_tmp_dst, password, recursive=True, timeout=20)
        # test_session.scp_send(file_to_send_modules, clr_tmp_dst, '', recursive=True, timeout=20)
        time.sleep(60)
        try:
