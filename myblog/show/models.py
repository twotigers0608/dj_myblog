...
class PullRequest(models.Model):
    ...
    latest_reviewset = models.IntegerField(default=1)
    owner = models.ForeignKey(Person, on_delete=models.CASCADE)
...
class PRSet(models.Model):
    jira = models.CharField(max_length=16)
    ...
    class Meta:
        unique_together = (('jira', 'reviewset'),)
...
class Patch(models.Model):
    ...
    # patch_hash = hash(subject + commit msg + payload)
    patch_hash = models.CharField(max_length=128, null=False, blank=False)
    payload_hash = models.CharField(max_length=10240, null=False, blank=False)
    payload = models.TextField(null=True, blank=True)
    subject = models.CharField(max_length=10240)
    commit_msg = models.TextField(null=True, blank=True)
    tags = models.TextField(null=True, blank=True)
    commit_id = models.CharField(max_length=128, null=True, blank=True)
    ...
    # patch review set number
    reviewsets = ArrayField(models.IntegerField(), null=True, blank=True)
    ...
    # only for review type: deleted patch
    main_patch = models.IntegerField(null=True, blank=True)
    # only for review type: updated patch
    a_ref = models.IntegerField(null=True, blank=True)
    same_payload_refs = ArrayField(models.IntegerField(), null=True, blank=True)
    ...
    def add_same_payload_refs(self, val):
        if self.same_payload_refs:
            if val not in self.same_payload_refs:
                self.same_payload_refs.append(val)
        else:
            self.same_payload_refs = [val]
...
# no patchset for refonly and removed patch
@python_2_unicode_compatible
class PatchSet(models.Model):
    patch = models.ForeignKey(Patch, on_delete=models.CASCADE)
    patchset = models.IntegerField(default=1)
    patch_hash = models.CharField(max_length=64, null=True, blank=True)
    ...
    class Meta:
        unique_together = (('patch', 'patchset'),)
...
class VoteRecord(models.Model):
    ...
    class Meta:
        unique_together = (('patch', 'patchset', 'voter'),)

    def __str__(self):
        return "%d/%d: %d" % (self.patch, self.patchset, self.id)
...
@python_2_unicode_compatible
class ReleasedPatch(models.Model):
